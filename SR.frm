...
Public skd As Integer '   Mass Seek and Destroy toggle
Public strt As Integer '  Demo Mode toggle
...
Private Sub Form_Load() ' initialize game / level
    If lv < 1 And Command$ = "/nosnd" Then SR.Check1.Value = 0 ' check for start muted command line option.
    Randomize Timer()
...
pb:
    ...
    If strt = 0 Then ch(t, 0) = ch(t, 0) * 250: ch(t, 1) = ch(t, 1) * 50: skd = 1
pb2:
...
pbb:
    ...
    If strt = 0 Then ch(t, 3) = ch(t, 3) * 30
...
pb2:
    ...
    GoTo pbb
paul:
     If (Y > yp - 480) And (Y <= yp + Shape3.Width) Then Label15.Visible = True: Image6(0).Visible = True: Image6(0).Top = X: _
                                                    Image6(0).Left = Y: uu = lv * 2: em(k, 0) = em(k, 0) - uu: bh = bh - ((Int(em(k, 1) / 700)) * lv): _
                                                    Label15.Caption = bh: Label15.Top = Shape3.Top + 100: Label15.Left = Shape3.Left + 100: rchngdir (k)
pbb:
    Next k
End Sub
...
End Sub
Private Sub Timer4_Timer() ' see if any of your charcters, or Bad Guys are still alive
...
pb2: ' all Bad guys are Dead, revive and  go to next level
   For paul = 0 To 6: Image2(paul).Top = Int(Rnd(1) * (SR.Height - 1800)): Image2(paul).Left = Int(Rnd(1) * (SR.Width - 2000))
   Next paul
   Form_Load
pb:
End Sub
Private Sub Form_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single) ' form clicked
...
dn:
End Sub
Private Sub Image3_Click(Index As Integer) ' Character Clicked
...
End Sub
Private Sub Text1_Change() ' Skill Level Change
...
clover:
    ...
    Sndfx ("gg1.snd")
    Label10.Caption = n(t) + " Gets " + itn(p) + " !!!!": Label10.ForeColor = vbBlack
    Label10.Top = Image3(t).Top - 100: Label10.Left = Image3(t).Left - 100
...
undead:
    ...
    Label10.ForeColor = vbBlue: Label10.Top = Image3(t).Top - 10: Label10.Left = Image3(t).Left - 100
    Label10.Caption = n(t) + itn(p) + n(z) + " Back to LIFE !!!": List1.AddItem (n(t) + itn(p) + n(z) + " Back to LIFE !!!"): ubx
    Image1(p).Visible = False: Sndfx ("gg2.snd")
    GoTo wo
foob:
    Next z: Label10.ForeColor = vbRed: Label10.Top = Image3(t).Top - 10: Label10.Left = Image3(t).Left - 100
    Label10.Caption = n(t) + " Can't Use this item right now!!!": GoTo wo

Bld:
    bh = lv * (10 * ch(z, 2))
    Label10.Caption = n(t) + " Builds a Safety Compound": Label10.ForeColor = vbBlack
    Label10.Top = Image3(t).Top - 100: Label10.Left = Image3(t).Left - 100: List1.AddItem ("Safety Compound Built"): ubx
...
End Sub
Private Sub Option1_Click() ' seek and destroy character option
...
dn:
End Sub
Private Sub seekdest(pb As Integer) ' Seek and Destroy routine
wow:
...
Private Sub Form_KeyPress(KeyAscii As Integer) ' Key control
    ...
    If KeyAscii = 103 Then GoTo mad ' "g" Mad Mode Cheat key
...
Private Sub Command1_Click() ' help
    ...
    Chr$(13) + "Skill Level and Game Speed are the Same Thing: Level 1 is Easiest (Slowest), 4 is Hardest (Fastest). GOOD LUCK !"
End Sub
...
Private Sub Label11_Click() 'contact info
    Label9_Click
End Sub
Private Sub Command2_Click() 'Clear Msg Log
    List1.Clear
End Sub
